...
- Phase 12: optional R&D pipeline telemetry (template codegen/validation/sign/attest)
  behind explicit feature flags with hash-only diagnostics.
"""
...
try:
    ...
    InMemoryChainAdapter = None
    run_rd_pipeline = None
...
class TopoCoreTCXv5AdvanceCASGit:
    ...
    @staticmethod
    def _rd_enabled(policy: dict[str, Any], limits: dict[str, Any]) -> bool:
        return (
            _to_bool(os.getenv("RB_TKYA_ENABLE_RD_PIPELINE", "0"))
            or _to_bool(policy.get("rd_enabled"), False)
            or _to_bool(limits.get("rd_enabled"), False)
        )

    @staticmethod
    def _rd_intent(task: str, policy: dict[str, Any], limits: dict[str, Any]) -> str:
        value = str(policy.get("rd_intent") or limits.get("rd_intent") or "").strip().lower()
        if value:
            return value
        if task == "review":
            return "pytest_test"
    ...
    def _run_rd(
        ...
        *,
        ...
        if not self._rd_enabled(policy, limits):
            return default_summary
        if run_rd_pipeline is None:
            return {**default_summary, "rd_status": "unavailable"}

        rd_intent = self._rd_intent(task, policy, limits)
        rd_context = policy.get("rd_context", {})
        if not isinstance(rd_context, dict):
            rd_context = {}

        if "function_name" not in rd_context:
            rd_context["function_name"] = _tokens(question)[0] if _tokens(question) else "generated_function"
        if "doc" not in rd_context:
            rd_context["doc"] = "Generated by TopoCore v5 R&D pipeline."

        rd_sandbox = policy.get("rd_sandbox", limits.get("rd_sandbox", {}))
        if not isinstance(rd_sandbox, dict):
            rd_sandbox = {}
        rd_policy = {"sandbox": rd_sandbox}

        signing_secret = os.getenv("RB_TKYA_RD_SIGNING_SECRET", "").strip() or None
        use_chain = _to_bool(os.getenv("RB_TKYA_RD_ENABLE_CHAIN", "0"), False)
        chain = self._rd_chain if use_chain else None
        run_id = str(
            policy.get("run_id")
            or limits.get("run_id")
            or os.getenv("GITHUB_RUN_ID", "")
            or "topocore-v5-local"
        )

        try:
            rd_result = run_rd_pipeline(
                intent=rd_intent,
                context=rd_context,
                policy=rd_policy,
            ...
            return {
                **default_summary,
                "rd_used": True,
                "rd_status": "error",
                "rd_intent_hash": _h(rd_intent, size=12),
            }

        diagnostics = dict(rd_result.diagnostics or {})
        attestation = dict(rd_result.attestation or {})
        rd_status = str(rd_result.status or "unknown")
        stable_token = _h(
            "|".join(
                [
                    diagnostics.get("intent_hash", _h(rd_intent, size=12)),
                    rd_result.codegen.template_id,
                    rd_result.validation.policy_hash,
                    rd_status,
                ]
            ),
            size=8,
        ...
        return {
            "rd_used": True,
            "rd_status": rd_status,
            "rd_template_id": str(rd_result.codegen.template_id),
            "rd_intent_hash": str(diagnostics.get("intent_hash", _h(rd_intent, size=12))),
            "rd_policy_hash": str(rd_result.validation.policy_hash),
            "rd_has_signature": rd_result.signature is not None,
            "rd_has_attestation": rd_result.attestation is not None,
            "rd_error_count": len(rd_result.validation.errors),
            "rd_warning_count": len(rd_result.validation.warnings),
            "rd_record_hash": str(attestation.get("record_hash", "n/a")),
            "rd_stable_token": stable_token,
        }
    ...
    def decide(self, req: EngineRequest) -> EngineDecision:
        ...
        rd_summary = self._run_rd(
            task=task,
            question=question,
            policy=policy,
            limits=limits,
        )
        if rd_summary.get("rd_used") and rd_summary.get("rd_status") == "blocked_validation":
            if route == "FAST":
                route = "DEEP"
            phase1["params"] = {
                **dict(phase1.get("params", {})),
                "rd_gate": "blocked_validation",
            }
        stable_tokens_raw = {
            _h(question, size=8),
        ...
        return EngineDecision(
            ...
            compression_stats={
                ...
                "trace": trace,
                "rd_used": bool(rd_summary.get("rd_used", False)),
                "rd_status": str(rd_summary.get("rd_status", "disabled")),
                "rd_template_id": str(rd_summary.get("rd_template_id", "n/a")),
                "rd_intent_hash": str(rd_summary.get("rd_intent_hash", "n/a")),
                "rd_policy_hash": str(rd_summary.get("rd_policy_hash", "n/a")),
                "rd_has_signature": bool(rd_summary.get("rd_has_signature", False)),
                "rd_has_attestation": bool(rd_summary.get("rd_has_attestation", False)),
                "rd_error_count": int(rd_summary.get("rd_error_count", 0)),
                "rd_warning_count": int(rd_summary.get("rd_warning_count", 0)),
                "rd_record_hash": str(rd_summary.get("rd_record_hash", "n/a")),
            },
            security=sec,
            rationale=(
                f"CoreLocked v5: {route} via Phase1 action {action}. "
