...
def test_review_and_verify_do_not_reenable_removed_v5_with_allow_flag(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv(TOPOCORE_V5_ALLOW_DEPRECATED_ENV, "1")

    with pytest.raises(TopoCoreBackendError) as review_exc:
        tky_local.LocalTKYProvider().compress_context(
            question="Review the fallback behavior.",
            candidates=_sample_candidates(),
            limits={"task_type": "review"},
        )
    with pytest.raises(TopoCoreBackendError) as verify_exc:
        tky_local.LocalTKYProvider().compress_context(
    ...
    assert V6_UNAVAILABLE_V5_DISABLED_REASON in str(review_exc.value)
    assert V6_UNAVAILABLE_V5_DISABLED_REASON in str(verify_exc.value)
