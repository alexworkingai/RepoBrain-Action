...
def test_refuse_short_circuits_without_second_pass(monkeypatch) -> None:
    retrieve_calls = {"n": 0}
    ...
    assert result.tky.route == "REFUSE"
    assert result.evidence == []
    assert audit["pass_count"] == 1
    assert audit["route_final"] == "REFUSE"


def test_block_short_circuits_without_second_pass(monkeypatch) -> None:
    retrieve_calls = {"n": 0}
