...
def test_v6_enriched_audit_output_shows_scorecard_and_safe_paths(tmp_path: Path) -> None:
    _make_repo(tmp_path)
    response = _valid_response()
    response["category_scores"][0]["evidence_paths"] = ["src/main.py"]
    response["score_adjustments"] = [
        {
            "category": "documentation",
            "delta": -1,
            "reason": "Docs coverage is shallow.",
            "evidence_paths": ["README.md", "docs/security.md"],
        }
    ]
    sys.modules["topocore_v6"] = _fake_topocore_module(response=response)
    audit: dict[str, object] = {}

    markdown = _build_audit_markdown(
        repo_root=tmp_path,
        query="Focus on partner-demo clarity.",
        tky_mode="local",
    ...
    assert "## Executive score card" in markdown
    assert "Audit mode: `v6-enriched scoring`" in markdown
    assert "Static baseline:" in markdown
    assert "v6 enriched score:" in markdown
    assert "Delta:" in markdown
    assert "Contract: `topocore.audit_score.v1` accepted" in markdown
    assert "src/main.py" in markdown
    assert "README.md" in markdown
    assert markdown.count("[redacted-path]") == 0
    assert audit["resolved_backend"] == "v6"


def test_audit_output_remains_static_when_capability_is_unavailable(tmp_path: Path) -> None:
    _make_repo(tmp_path)
    sys.modules.pop("topocore_v6", None)
    markdown = _build_audit_markdown(
        repo_root=tmp_path,
...
def test_audit_output_rejects_invalid_v6_response_without_raw_json(tmp_path: Path) -> None:
    ...
    invalid["safety"]["patch_applied"] = True
