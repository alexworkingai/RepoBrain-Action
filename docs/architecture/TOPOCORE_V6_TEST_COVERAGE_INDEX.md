...
| `tests/test_sprint90_decisive_installed_package_proof_docs.py` | Sprint 90 decisive-proof architecture and readiness docs | Phase 7 | Live installed-package proof pass state, parser readiness transition, and no-fake-switch truth remain explicit | No |
| `tests/test_decisive_artifact_token_preflight_docs.py` | Sprint 90 token/preflight docs | Phase 7 | Secret name, scope, expiration, no-value handling, and decisive preflight pass evidence remain explicit | No |
| `tests/test_public_switch_ready_decision_after_sprint90.py` | Sprint 90 public-readiness docs | Phase 7 | Ready-after-runtime-proof becomes current only after live proof, while no false switch or Marketplace claim is introduced | No |
...
| `tests/test_justified_pr_comment_permission_classification.py` | `repobrain/workflow_permission_classifier.py` | Phase 7 | Workflow permission classes distinguish safe read-mostly, justified PR comment response permission, write-heavy review-required, and dangerous policy cases | No |
| `tests/test_diagnostics_output_tiers.py` | `repobrain/output_md.py` | Phase 7 | Default GitHub comments hide raw TKY/TKYA internals while verbose mode can still expose expanded sanitized diagnostics | No |
| `tests/test_compact_llm_block.py` | `repobrain/output_md.py` | Phase 7 | Compact default LLM block reports used/not-called truth, final model, token totals, and avoids quota/planner noise | No |
| `tests/test_compact_runtime_safety_block.py` | `repobrain/output_md.py` | Phase 7 | Compact runtime/safety block hides raw TKY/TKYA fields and replaces repeated boolean matrices with partner-facing safety wording | No |
| `tests/test_command_output_shapes_compact.py` | `repobrain/output_md.py` | Phase 7 | Ask/review/locate compact output shapes preserve useful evidence and route truth without noisy runtime internals | No |
| `tests/test_audit_permission_scoring_justification.py` | `repobrain/audit_scoring.py` | Phase 7 | Audit and score no longer overstate justified `pull-requests: write`, while broad write/dangerous workflow policies still penalize normally | No |
...
- public-readiness docs preserve the historical owner-action gate rather than drifting before real proof exists
