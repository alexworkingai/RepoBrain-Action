...
| External GitHub mode foundation | third-party caller workflow -> `repobrain_external_foundation.yml` | `help`, `doctor`, `ask`, `review`, `fix` | out-of-contract commands | Product-shaped third-party GitHub-native foundation path with bounded read-only review and Fix-Lite suggestion-only mode. |
