...
def test_issue_ask_product_analysis_stays_on_target_repo(
    ...
    assert "What this repository appears to be:" in answer_text
    assert "Evidence-backed runtime/application surfaces:" in answer_text
    assert "Evidence-backed RepoBrain connection boundary:" in answer_text
    assert "Partner pilot checks based only on selected evidence:" in answer_text
    assert "Limitations / not confirmed in selected evidence:" in answer_text
    ...
    assert "docs/ENTERPRISE_READINESS.md" not in override_paths
