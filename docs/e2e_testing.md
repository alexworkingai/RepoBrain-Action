...
- Fix prompts enforce strict patch contract:
  - primary: ` ```diff ... ``` `
  - fallback: raw unified diff
  - provider-compat fallback: JSON envelope `{"result":"patch","diff":"..."}` or `{"result":"no_patch"}`
  - explicit no-change outcome: `NO_PATCH`
